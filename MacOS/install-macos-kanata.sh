...
Action needed:
  Enable kanata in System Settings -> Privacy & Security -> Accessibility.
  If kanata is not listed, add this binary with the + button:
    $KANATA_BIN
...
EOF
  ...
  wait_for_user "Enable kanata in Accessibility, then press Enter to start services, or type q to quit: "
}
...
plutil -lint "$TMP_DIR/$KANATA_LABEL.plist" "$TMP_DIR/$HELPER_LABEL.plist" >/dev/null
